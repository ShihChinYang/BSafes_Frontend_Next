...
export function decryptLargeBinaryString(binaryString, key, iv) {
  ...
  while (1) {
    ...
    if (end >= (binaryString.length)) {
      ...
      start = end;
      if(!decryptedSlice){
        debugLog(debugOn, `decrypt error`);
      }
      debugLog(debugOn, `slice length: ${sliceStr.length}, decrypted slice length: ${decryptedSlice.length}, decrypted string length:${decryptedString.length}`);
    }
  }
...
export async function calculateAccountHash(password, keySalt) {
  let newBytes = forge.util.encode64(keySalt);
  newBytes = newBytes.replace(/\+/g, '0');

  const result = await new Promise((resolve, reject) => {
    bcrypt.hash(password, "$2a$10$" + newBytes, function (err, hash) {
      if (err) {
        reject(null);
      } else {
        resolve(forge.util.encode64(hash));
      }
    });
  });
  return result;
...
export async function calculateCredentials(nickname, password, logInOnly = false) {
  ...
  try {
    ...
    result = await calculateAccountHash(password, keySalt);
    credentials.keyPack.accountHash = result;
...
export function saveLocalCredentials(credentials, sessionKey, sessionIV, accountVersion = 'v2') {
  ...
  localStorage.setItem("authId", credentials.keyPack.id);
  localStorage.setItem("memberId", credentials.memberId);
  localStorage.setItem("displayName", credentials.displayName);
  localStorage.setItem("currentKeyVersion", credentials.currentKeyVersion);
  ...
  if (accountVersion === 'v1') {
    ...
    localStorage.setItem("keySalt", forge.util.encode64(credentials.secret.keySalt));
  }
...
export function readLocalCredentials(sessionKey, sessionIV) {
  ...
  credentials.keyPack.id = localStorage.getItem("authId");
  credentials.memberId = localStorage.getItem("memberId");
  credentials.displayName = localStorage.getItem("displayName");
  credentials.currentKeyVersion = localStorage.getItem("currentKeyVersion");
  ...
  if (accountVersion === 'v1') {
    ...
    credentials.secret.keySalt = forge.util.decode64(localStorage.getItem("keySalt"));
  }

  return credentials;
}

export function createAccountRecoveryCode(nickname, keyPassword, clientEncryptionKey) {
  const nickNameLength = nickname.length;
  const thisString = `${nickNameLength}:${nickname}${keyPassword}`;
  const accountRecoveryPhrase = forge.util.encode64(encryptBinaryString(thisString, clientEncryptionKey));
  return accountRecoveryPhrase;
}

export function readAccountRecoveryCode(accountRecoveryCode, clientEncryptionKey) {
  try {
    const thisString = decryptBinaryString(forge.util.decode64(accountRecoveryCode), clientEncryptionKey);
    const nickNameLengthStr = thisString.split(':')[0];
    const otherString = thisString.substring(nickNameLengthStr.length + 1);
    const nickNameLength = parseInt(nickNameLengthStr);
    const nickname = otherString.substring(0, nickNameLength);
    const keyPassword = otherString.substring(nickNameLength);
    return { nickname, keyPassword };
  } catch (error) {
    return { nickname: '', keyPassword: '' };
  }
}
