...
export function getDemoUrl(productId) {
  if (productId && productId !== "") {
    let demoUrl;
    if(tryMeSamples[productId]){
      demoUrl = tryMeSamples[productId].demoUrl;
      return demoUrl;
    }
    const type = productTypes[productId.charAt(0)];
    demoUrl = `/${type}Demo/${type.charAt(0)}:_pd_${productId}_pd_${demoOwner}:3:1747621297857`
    return demoUrl;
  } else {
    return '/safe';
  }
