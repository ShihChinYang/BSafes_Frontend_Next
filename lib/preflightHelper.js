...
import { setAccountVersion, setDisplayName, loggedIn, loggedOut, setFroalaLicenseKey, setClientEncryptionKey, setPreflightReady, setPreflightResponseProcessed, clearLocalDataAndIndexDB } from '../reduxStore/auth';
import { setNextAuthStep, setKeyMeta } from '../reduxStore/v1AccountSlice';

export async function processPreflightResponse(data, dispatch, combinedPreflight = false) {
    if (data.status === 'ok') {
