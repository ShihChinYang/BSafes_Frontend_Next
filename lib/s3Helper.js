...
export function preS3ChunkDownload(itemId, chunkIndex, s3KeyPrefix, numberOfChunksRequired, dispatch) {
    return new Promise((resolve, reject) => {
        let s3Key, signedURL, numberOfChunks;
        if (process.env.NEXT_PUBLIC_app !== "localBackup") {
            PostCall({
                ...
                if (data.status === 'ok') {
                    ...
                    s3Key = data.s3Key;
                    signedURL = data.signedURL;
                    resolve({ s3Key, signedURL, numberOfChunks });
                } else {
...
export function getS3Object(signedURL, progressCallback = null) {
    return new Promise(async (resolve, reject) => {
        ...
        xhr.onload = function (e) {
            if (xhr.status === 200) {
                const buffer = Buffer.from(this.response, 'binary');
                const downloadedBinaryString = buffer.toString('binary');
                resolve({ binaryString: downloadedBinaryString, arraryBuffer: this.response });
            } else {
                reject();
            }
        };
