...
export default function PageCommonControls({ showWriteBtn = true, isEditing, onWrite, readyForSaving = true, onSave, onCancel, canEdit = true }) {
    ...
    return (
        <>
            ...
            {(isEditing && readyForSaving) ? <Button onClick={onSave} className={`${BSafesStyle.btnCircle} ${BSafesStyle.btnFloating} ${BSafesStyle.btnFloatingSave}`} ><i className="fa fa-check" aria-hidden="true"></i></Button> : <></>}
            {isEditing ? <Button onClick={onCancel} className={`${BSafesStyle.btnCircle} ${BSafesStyle.btnFloating} ${BSafesStyle.btnFloatingCancel}`}><i className="fa fa-times" aria-hidden="true"></i></Button> : <></>}
        </>
    )
}